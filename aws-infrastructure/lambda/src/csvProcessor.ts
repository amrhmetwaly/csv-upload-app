import { ProcessedData, UsageRecord, SummaryData } from './types';
...
export class CSVProcessor {
  static processCSVContent(fileContent: string, threshold: number): ProcessedData {
    ...
    const dateColumns = headers.filter(header => 
      /date|day|month|time|period|billing/i.test(header)
    );
  ...
  static validateCSVFile(fileName: string, fileSize: number): void {
